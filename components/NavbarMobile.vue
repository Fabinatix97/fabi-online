<template>
  <Disclosure
    v-slot="{ open, close }"
    as="nav"
    class="navbar border-b-[1px] border-[var(--border)] hyphens-none"
  >
    <div class="flex items-start">
      <DisclosureButton
        :aria-label="open ? 'Hauptmenü schließen' : 'Hauptmenü öffnen'"
        class="rounded-2xl p-2 text-[var(--heading)] hover:bg-[var(--body)] hover:text-[var(--primary)] focus:ring-2 focus:ring-white focus:outline-none focus:ring-inset"
      >
        <template v-if="!open">
          <!-- Open Main Menu -->
          ...
          <NuxtLink
            ...
            :class="[
              isActiveRoute(item.href)
                ? 'bg-[var(--primary)] text-[var(--body)] hover:text-[var(--heading)]'
                : 'hover:bg-[var(--main)] hover:text-[var(--primary)]',
        ...
        <div class="flex px-2">
          <div class="mr-4">
            <a
              ...
              class="transition-all duration-300 hover:text-[var(--primaryhover)]"
